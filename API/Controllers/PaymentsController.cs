    ...
    ILogger<PaymentsController> logger) : BaseApiController
{
    ...
    private async Task HandlePaymentIntentFailed(PaymentIntent intent)
    {
        ...
        if (order == null)
        {
            logger.LogWarning("Payment intent {PaymentIntentId} failed but no matching order was found", intent.Id);
            return;
        }
        ...
        foreach (var item in order.OrderItems)
        {
            ...
            if (productItem == null)
            {
                logger.LogWarning("Payment intent {PaymentIntentId} failed but product {ProductId} was not found to revert stock",
                    intent.Id, item.ItemOrdered.ProductId);
                continue;
            }
    ...
    private async Task HandlePaymentIntentSucceeded(PaymentIntent intent)
    {
        ...
        if (order == null)
        {
            logger.LogWarning("Payment intent {PaymentIntentId} succeeded but no matching order was found", intent.Id);
            return;
        }
